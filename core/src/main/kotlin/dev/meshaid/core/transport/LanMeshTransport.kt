...
class LanMeshTransport(
    ...
    private fun startDiscovery() {
        ...
        runCatching { socket.joinGroup(InetSocketAddress(group, multicastPort), null) }
        beaconSocket = socket
        ...
        thread("lan-beacon-tx") {
            val payload = ByteBuffer.allocate(MAGIC.size + 8 + 2)
                ...
                .array()
            while (running) {
                runCatching { socket.send(DatagramPacket(payload, payload.size, group, multicastPort)) }
                try {
        ...
        thread("lan-beacon-rx") {
            ...
            while (running) {
                ...
                // Same dial rule as BLE: smaller id initiates, so each pair gets one link.
                if (selfId.raw.toULong() >= peerId.raw.toULong()) continue
                connectTo(packet.address.hostAddress ?: continue, peerPort)
            }
        }
    }
